...
f = open('datastructureQs_extra.json', 'w')
f.write("{\n")

for i in range(1,42):
	page = requests.get('http://www.careerride.com/question-' + str(i) + '-C')
	...
	print(question)
	print(answers)
	if len(max(answers, key=len)) <= 20:
		answers = json.dumps(answers)
		print("THIS IS GOING TO PRINT")
		f.write('    "(working in C) ' + str(question[0]) + '":\n        ' + str(answers) + ",\n")
	else:
		print("8888888888")
